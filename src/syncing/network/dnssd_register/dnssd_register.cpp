...
void DnssdRegister::publishCallback(
  ...
  if (errorCode != kDNSServiceErr_NoError) {
    emit registerObj->OnServiceRegisteringFailed(std::make_exception_ptr(common::types::exceptions::ErrorCodeException(static_cast<int>(errorCode), "Service Registering Failed")));
  }
 ...
 */
DnssdRegister::DnssdRegister(QObject* parent) : Register(parent) {
...
void DnssdRegister::registerService(int port) {
  QByteArray serviceName = constants::getMDnsServiceName().toUtf8();
  QByteArray serviceType = constants::getMDnsServiceType().toUtf8();
  ...
  auto errorType = DNSServiceRegister(
      ...
      0,                                        // flags
      serviceName.constData(),                  // server's name
      serviceType.constData(),                  // service type
      NULL,                                     // domain
      NULL,                                     // host
      htons(port),                              // port
      0,                                        // txtLen
  ...
  if (errorType != kDNSServiceErr_NoError) {
    emit this->OnServiceRegisteringFailed(std::make_exception_ptr(common::types::exceptions::ErrorCodeException(static_cast<int>(errorType), "Service Registering Failed")));
  }
...
void DnssdRegister::unregisterService() {
  ...
  emit this->OnServiceUnregistered();
}

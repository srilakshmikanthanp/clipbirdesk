...
TEST(ToBytesTest, TestingToBytes) {
  ...
  // int64_t
  int64_t i64_t = 0x123456789ABCDEF0;
  std::vector<std::uint8_t> o64_t = {0xF0, 0xDE, 0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12};

  EXPECT_EQ(toBytes(i64_t), o64_t);

  // int16_t
  int16_t i16_t = 0x1234;
  std::vector<std::uint8_t> o16_t = {0x34, 0x12};

  EXPECT_EQ(toBytes(i16_t), o16_t);

  // int8_t
  int8_t i8_t = 0x12;
  std::vector<std::uint8_t> o8_t = {0x12};

  EXPECT_EQ(toBytes(i8_t), o8_t);

  // int32_t
  int32_t i32_t = 0x12345678;
  std::vector<std::uint8_t> o32_t = {0x78, 0x56, 0x34, 0x12};
...
TEST(FromBytesTest, TestingFromBytes) {
  ...
  // int16_t
  std::vector<std::uint8_t> i16_t = {0x34, 0x12};
  int16_t o16_t = 0x1234;

  EXPECT_EQ(fromBytes<int16_t>(i16_t), o16_t);

  // int8_t
  std::vector<std::uint8_t> i8_t = {0x12};
  int8_t o8_t = 0x12;

  EXPECT_EQ(fromBytes<int8_t>(i8_t), o8_t);

  // int32_t
  std::vector<std::uint8_t> i32_t = {0x78, 0x56, 0x34, 0x12};
